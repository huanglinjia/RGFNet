...
class Encoder(nn.Module):
    def __init__(self, input_dim, emb_dim, hid_dim, n_layers, dropout):
        ...
        config = BertConfig.from_pretrained("weight/bert_config.json")
        self.embedding = BertModel.from_pretrained("weight/pytorch_model.bin", config=config)
    ...
    def forward(self, src, input_length):
        ...
        embedded = self.embedding(src.transpose(0, 1), attention_mask=seq_mask)
        output = self.dropout(embedded[0]).transpose(0, 1)

        feed_hidden = output[-1, :, :] + output[0, :, :]

        return output, feed_hidden


class BSeq(nn.Module):
    def __init__(self, input_dim, emb_dim, hid_dim, n_layers, output_size, dropout):
        super().__init__()
    ...
    def forward(self, src, input_length, trg):
...
class DotAttn(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs, seq_mask=None, dropout=None):
        attn_energies = hidden.transpose(0, 1).bmm(encoder_outputs.permute(1, 2, 0))
