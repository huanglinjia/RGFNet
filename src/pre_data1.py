...
def transfer_ape_num(data, isTrans=False):  # transfer num into "NUM"
    print("Transfer numbers...")
    pattern = re.compile("\d*\(\d+/\d+\)\d*|\d+\.\d+%?|\d+%?|\d+<\.\d+>?")
    gen=0
    pairs = []
    ...
    for d in data:
        ...
        for i, s in enumerate(seg):
            ...
            if '(' == s or '（' == s:
                ...
                if i + k + 3 <= len(seg) - 1:
                    com = seg[i + k - 1] + seg[i + k] + seg[i + k + 1] + seg[i + k + 2] + seg[i + k + 3]
                    pos = re.search(pattern, com)
                    if pos and pos.start() == 0:
                        wait = wait[:-1]
                        input_seq += wait
                        nums.append(s[pos.start(): pos.end()])
                        input_seq.append("NUM")
                        count = len(wait) + len(com) - 1
                        is_count = True
                        continue
                    else:
                        count = len(wait) - 1
        ...
        pairs.append((input_seq, out_seq, nums, num_pos))
    temp =[]
    for n in generate_nums:
        if generate_nums_dict[n]>=10:
            temp.append(n)
    return pairs, temp, copy_nums
