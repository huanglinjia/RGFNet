...
def train(input_batch, input_length, output_batch, output_length, num_stack_batch, output_lang, num_pos, batch_graph,
    ...
    output = model.forward(input_var, input_length, trg, num_pos, batch_graph, output_lang, teacher_forcing, num_stack_batch)
    trg = trg[1:]
...
def evaluate(input_batch, input_length, output_batch, output_length, output_lang, num_pos, batch_graph, model):
    model.eval()
    ...
    if USE_CUDA:
        ...
        trg = trg.cuda()
        batch_graph = batch_graph.cuda()
    output = model.forward(input_var, input_length, trg, num_pos, batch_graph, output_lang, -1)
    trg = trg[1:]
    loss = masked_cross_entropy(output.transpose(0, 1).contiguous(), trg.transpose(0, 1).contiguous(), output_length)
