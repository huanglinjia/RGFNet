...
def transfer_num(data):  # transfer num into "NUM"
    ...
    for d in data:
        ...
        for i, j in enumerate(input_seq):
            if j == "NUM":
                num_pos.append(i)
        for i, j in enumerate(word_seq):
            if j == "NUM":
                word_num_pos.append(i)

        assert len(nums) == len(num_pos)
        # pairs.append((input_seq, out_seq, nums, num_pos, d["ans"]))
        pairs.append((input_seq, out_seq, nums, num_pos, word_seq, word_num_pos))
...
def prepare_data(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    ...
    for pair in pairs_trained:
        if not tree:
            ...
            input_lang.add_sen_to_vocab(pair[4])
            output_lang.add_sen_to_vocab(pair[1])
        elif pair[-1]:
    ...
    for pair in pairs_trained:
        ...
        input_cell = indexes_from_sentence(input_lang, pair[0])
        word_input_cell = indexes_from_sentence(input_lang, pair[4])
        output_cell = indexes_from_sentence(output_lang, pair[1], tree)

        # train_pairs.append((input_cell, len(input_cell), output_cell, len(output_cell),
        #                     pair[2], pair[3], num_stack, pair[4]))
        train_pairs.append((input_cell, len(input_cell), output_cell, len(output_cell),
                            pair[2], list(np.array(pair[3])+1), num_stack))

        if len(input_cell) != len(word_input_cell):
            output_cell_1 = copy.deepcopy(output_cell)
            n1 = copy.deepcopy(pair[2])
            n2 = copy.deepcopy(num_stack)
            train_pairs.append((word_input_cell, len(word_input_cell), output_cell_1, len(output_cell_1),
                            n1, list(np.array(pair[5])+1), n2))
