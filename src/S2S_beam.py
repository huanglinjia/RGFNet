...
class Decoder(nn.Module):
    ...
    def forward(self, input_ids, last_hidden, word_dict_vec, encoder_outputs, seq_mask):
        ...
        rnn_output, hidden = self.gru(torch.cat((last_embedded.unsqueeze(0), context.transpose(0, 1)), 2),
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, input_length, trg, num_pos, output_lang, teacher_forcing):
        ...
        encoder_out, encoder_hidden = self.encoder.forward(src, input_length)
        ...
        if teacher_forcing > random.random():
            ...
            for t in range(0, max_len - 1):
                output, decoder_hidden = self.decoder(trg[t, :], decoder_hidden, word_dict_vec, encoder_out, seq_mask)
                out = self.score(output.unsqueeze(1), word_dict_vec, num_mask)
                out_list.append(nn.functional.log_softmax(out, dim=-1))
            ...
            for t in range(max_target_length - 1):
                ...
                for b_idx in range(len(beam_list)):
                    ...
                    decoder_output, decoder_hidden = self.decoder(
                        decoder_input, decoder_hidden, word_dict_vec, encoder_out, seq_mask)

                    decoder_output = self.score(decoder_output.unsqueeze(1), word_dict_vec, num_mask)
                    score = nn.functional.log_softmax(decoder_output, dim=-1)
            ...
            decoder_out = nn.functional.log_softmax(beam_list[0].all_output, dim=-1)
