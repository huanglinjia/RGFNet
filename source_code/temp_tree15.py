...
#         z = torch.sigmoid(self.z_gates[2](torch.cat((f1, c), -1)))
#         c_z += 2 * c * z
#         f1 = finput - c * z
#         c = torch.relu(self.c_gates[3](torch.cat((pinput, context), -1)))
#         z = torch.sigmoid(self.z_gates[3](torch.cat((f1, c), -1)))
#         f1 = finput - c * z
#         z = torch.sigmoid(self.z_gates[3](torch.cat((f1, c), -1)))
#         c_z += c * z
#
#         return self.norm(finput - c_z / 6.0)
#
#     # def forward(self, pinput, context):
#     #     finput = torch.relu(self.i_gates(torch.cat((pinput, context), -1)))  # information merge
#     #
...
#         self.combine_pr = nn.Linear(hidden_size * 2, hidden_size)  # MinusGate(hidden_size, 1)
#         self.combine = nn.Linear(hidden_size * 2, hidden_size)  # MinusGate(hidden_size, 1)
#         self.combine_p = nn.Linear(hidden_size * 2, hidden_size)  # MinusGate(hidden_size, 1)
#
#         self.attn_feed = Attn(hidden_size)
...
class MinusGate(nn.Module):
    ...
    def forward(self, pinput, context):  # context 作为消融物
        # f1 = torch.relu(self.i_gates(torch.cat((pinput, context), -1)))  # information merge

        if self.use_pos:
            pinput = pinput * math.sqrt(self.hidden_size)
            pinput = pinput + self.pe[:pinput.size(0)]
            # length, batch, _ = pinput.size()
            context = context * math.sqrt(self.hidden_size)
            context = context + self.pe[pinput.size(0)-1]
            # pinput = self.bn_pos_1(pinput.view(-1, self.hidden_size)).view(length, batch, self.hidden_size)
            # context = self.bn_pos_2(context.view(-1, self.hidden_size)).view(length, batch, self.hidden_size)
            # pinput = self.bn_pos_1(pinput)
            # context = self.bn_pos_2(context)
        ...
        z = torch.sigmoid(self.z_gates[0](torch.cat((c, context), -1)))
        fu1 = c * z
        f1 = self.norms[1](finput - fu1)

        f1 = self.norms[0]((1 - torch.sigmoid(self.i_gates(torch.cat((f1, context), -1)))) * f1)
        c = torch.relu(self.c_gates[0](torch.cat((f1, context), -1)))
        z = torch.sigmoid(self.z_gates[0](torch.cat((c, context), -1)))
        fu2 = c * z

        return self.norms[2](finput - (fu1 + fu2) * 0.5)

    # def forward(self, pinput, context):
    #     finput = torch.relu(self.i_gates(torch.cat((pinput, context), -1)))  # information merge
    #
    #     c_z = 0
    #     c = torch.relu(selc_gates[0](torch.cat((pinput, context), -1)))
    #     z = torch.sigmoid(self.z_gates[0](torch.cat((finput, c), -1)))
    #     f1 = finput - c * z
    #     c = torch.relu(self.c_gates[1](torch.cat((pinput, context), -1)))
    #     z = torch.sigmoid(self.z_gates[1](torch.cat((0.5 * f1, c), -1)))
...
class Decoder(nn.Module):
    def __init__(self, hidden_size, operator_index, n_layers=2, dropout=0.5):
        ...
        self.remain = MinusGate(hidden_size)
        # self.remain1 = MinusGate(hidden_size)
        self.gf = MinusGate(hidden_size, use_pos=True)
        self.g1 = MinusGate(hidden_size)
        # self.g2 = MinusGate(hidden_size)
        # self.g3 = MinusGate(hidden_size)
        self.out_l = MinusGate(hidden_size)
            ...
            if len(nodes_parent_idx) >= 3:
                ...
                if l_index == r_index and bool_p_idx[-1] == 1 and bool_p_idx[-2] == 1:
                    while l_index == r_index and bool_p_idx[-1] == 1 and bool_p_idx[-2] == 1:
                        ...
                        #                                            nodes_word_dict_vector_idx[r_node].unsqueeze(0)),
                        #                                           -1))
                        c = torch.relu(self.combine(torch.cat((nodes_word_dict_vector_idx[l_node].unsqueeze(0),
                                                               nodes_word_dict_vector_idx[r_node].unsqueeze(0), \
                                                               nodes_word_dict_vector_idx[p_node].unsqueeze(0)), -1)))
                        ...
                        del c
                        c = torch.relu(self.combine_plr(
                            torch.cat((feed_hidden_idx[l_node].unsqueeze(0), feed_hidden_idx[r_node].unsqueeze(0), \
