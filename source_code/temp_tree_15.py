...
#
#         f1 = finput - fu
#         context = context + torch.tanh(self.g_gates[0](torch.cat((context, fu), -1))) * fu
#
#         c = torch.relu(self.c_gates[1](torch.cat((f1, context), -1)))
#         z = torch.sigmoid(self.z_gates[1](torch.cat((c, context), -1)))
#
#         fu1 = self.norms[2](c * z)
#         f2 = f1 - fu1
#
#         return f2
#     #     c_z = 0
...
#     #     f1 = finput - c * z
#     #     c = torch.relu(self.c_gates[1](torch.cat((pinput, context), -1)))
#     #     z = torch.sigmoid(self.z_gates[1](torch.cat((0.5 * f1, c), -1)))
#     #     f2 = finput - c * z
#     #     c = torch.relu(self.c_gates[2](torch.cat((pinput, context), -1)))
#     #     z = torch.sigmoid(self.z_gates[2](torch.cat((0.5 * f2, c), -1)))
#     #     f3 = finput - c * z
#     #     c = torch.relu(self.c_gates[3](torch.cat((pinput, context), -1)))
#     #     z = torch.sigmoid(self.z_gates[3](torch.cat((f3, c), -1)))
#     #     f4 = finput - c * z
...
#         self.combine_tpl = nn.Linear(hidden_size * 2, hidden_size)
#         self.combine_tpr = nn.Linear(hidden_size * 2, hidden_size)
#         self.combine_pl = nn.Linear(hidden_size * 2, hidden_size)
#         self.combine_pr = nn.Linear(hidden_size * 2, hidden_size)
...
#                         c = torch.relu(self.combine_tpl(torch.cat((nodes_word_dict_vector_idx[p_node].unsqueeze(0),
#                                                                    nodes_word_dict_vector_idx[l_node].unsqueeze(0)),
#                                                                   -1))) + nodes_word_dict_vector_idx[p_node].unsqueeze(0)
#                         #c = nodes_word_dict_vector_idx[p_node].unsqueeze(0)
#                         c = torch.relu(self.combine_tpr(torch.cat((c,
...
#
#                         del c
#                         c = torch.relu(self.combine_pl(torch.cat((feed_hidden_idx[p_node].unsqueeze(0),
#                                                                   feed_hidden_idx[l_node].unsqueeze(0)), -1))) + feed_hidden_idx[p_node].unsqueeze(0)
#                         c = torch.relu(self.combine_pr(torch.cat((c,
#                                                                   feed_hidden_idx[r_node].unsqueeze(0)), -1))) + c
...
#                     bool_parent[i] = 0
#                     c = torch.relu(self.combine(torch.cat((nodes_word_dict_vector_idx[r_index].unsqueeze(0),
#                                                            nodes_word_dict_vector_idx[r_node].unsqueeze(0)), -1))) + nodes_word_dict_vector_idx[r_index].unsqueeze(0)
#                     nodes_word_dict_vector_idx[r_index] = c
#                     parent_embedded.append(nodes_word_dict_vector_idx[r_node])
#
#                     c = parent_r_idx[r_node].r_embedding
#                     c = torch.relu(self.combine_p(torch.cat((c.unsqueeze(0),
#                                                              feed_hidden_idx[r_node].unsqueeze(0)), -1))) + c.unsqueeze(0)
#
...
#                     c = torch.relu(self.combine_p(torch.cat((c.unsqueeze(0),
#                                                              feed_hidden_idx[-1].unsqueeze(0)), -1))) + c.unsqueeze(0)
#                     parent_feed_hidden.append(c.squeeze(0))
#                     del c
#                     c = torch.relu(
#                         self.combine_tpl(torch.cat((nodes_word_dict_vector_idx[nodes_parent_idx[-1][0]].unsqueeze(0),
#                                                     nodes_word_dict_vector_idx[-1].unsqueeze(0)), -1))) + nodes_word_dict_vector_idx[nodes_parent_idx[-1][0]].unsqueeze(0)
...
class MinusGate(nn.Module):
    ...
    def forward(self, pinput, context): #context 作为消融物
        ...
        # if self.use_pos:
        #     f1 = f1 * math.sqrt(self.hidden_size)
        #     f1 = f1 + self.pe[:pinput.size(0)]

        f1 = self.norms[0]((1 - torch.sigmoid(self.i_gates(torch.cat((f1, context), -1)))) * f1)  # 信息映射
        c = torch.relu(self.c_gates[1](torch.cat((f1, context), -1)))
        z = torch.sigmoid(self.z_gates[1](torch.cat((c, context), -1)))
        fu2 = self.norms[2](c * z)
        # f2 = finput - 0.5 * fu2
        f2 = f1 - fu2

        # if self.use_pos:
        #     f2 = f2 * math.sqrt(self.hidden_size)
        #     f2 = f2 + self.pe[:pinput.size(0)]

        f2 = self.norms[0]((1 - torch.sigmoid(self.i_gates(torch.cat((f2, context), -1)))) * f2)  # 信息映射
        c = torch.relu(self.c_gates[2](torch.cat((f2, context), -1)))
        z = torch.sigmoid(self.z_gates[2](torch.cat((c, context), -1)))
        fu3 = self.norms[3](c * z)
        f3 = f2 - fu3

        # if self.use_pos:
        #     f3 = f3 * math.sqrt(self.hidden_size)
        #     f3 = f3 + self.pe[:pinput.size(0)]

        f3 = self.norms[0]((1 - torch.sigmoid(self.i_gates(torch.cat((f3, context), -1)))) * f3)  # 信息映射
        c = torch.relu(self.c_gates[3](torch.cat((f3, context), -1)))
        z = torch.sigmoid(self.z_gates[3](torch.cat((c, context), -1)))
        fu4 = self.norms[4](c * z)

        return f3 - fu4#finput - (fu1 + 2 * fu2 + 2 * fu3 + fu4) / 6.0
